...
const authStyles = {
  ...
  title: { position: 'absolute', top: '245px', left: '40px', fontSize: '45px', letterSpacing: '3px', color: '#2d2d5a', zIndex: 3, fontWeight: 'bold', fontFamily: '"Bodoni Moda", serif', fontStyle: 'italic' },
...
const homeStyles = {
  ...
  screen: { width: '100%', maxWidth: '375px', minHeight: '100dvh', position: 'relative', background: 'linear-gradient(to bottom, #201938, #9a8fd0)', overflow: 'hidden' }, // ← ここで背景グラデーションを設定しています
    ...
    try {
      if (isSignUp) {
        const { error } = await supabase.auth.signUp({
          email, password, options: { data: { username: searchId, search_id: searchId } },
        });
